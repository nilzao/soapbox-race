...
public class EventDataDao extends SoapboxDao {
	...
	public Long getNextSessionId() {
		...
		if (queryResult == null) queryResult = 0L;
	...
	public List<EventDataEntity> getRacers(Long eventSessionId) {
		EntityManager manager = getManager();
		TypedQuery<EventDataEntity> query = manager
				.createQuery("SELECT obj FROM EventDataEntity obj WHERE obj.eventSessionId = :eventSessionId", EventDataEntity.class);
	...
	public EventDataEntity findByEventSessionIdAndPersonaId(Long eventSessionId, Long personaId) {
		EntityManager manager = getManager();
		TypedQuery<EventDataEntity> query = manager
				.createQuery("SELECT obj FROM EventDataEntity obj WHERE obj.eventSessionId = :eventSessionId AND obj.personaId = :personaId", EventDataEntity.class);
		query.setParameter("eventSessionId", eventSessionId);
	...
	public List<EventDataEntity> findByEventId(Long eventId) {
		EntityManager manager = getManager();
		TypedQuery<EventDataEntity> query = manager
				.createQuery("SELECT obj FROM EventDataEntity obj WHERE obj.eventId = :eventId", EventDataEntity.class);
